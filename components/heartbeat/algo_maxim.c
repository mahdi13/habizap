// Minimal C port of Maxim's spo2_algorithm for 32-bit buffers at 25 Hz
// Based on external/src/spo2_algorithm.cpp with Arduino dependencies removed.
...
{
  if (!pun_ir_buffer || !pun_red_buffer || n_ir_buffer_length <= 0) {
    if (pn_spo2) *pn_spo2 = -999;
    if (pch_spo2_valid) *pch_spo2_valid = 0;
    if (pn_heart_rate) *pn_heart_rate = -999;
    if (pch_hr_valid) *pch_hr_valid = 0;
    return;
  }
  ...
  if (n_npks>=2){
    ...
    if (pn_heart_rate) *pn_heart_rate = (int32_t)((FreqS*60)/ n_peak_interval_sum );
    if (pch_hr_valid) *pch_hr_valid  = 1;
  }
  else  { 
    if (pn_heart_rate) *pn_heart_rate = -999; // unable to calculate because # of peaks are too small
    if (pch_hr_valid) *pch_hr_valid  = 0;
  }
  ...
  for (k=0; k< n_exact_ir_valley_locs_count; k++){
    if (an_ir_valley_locs[k] > BUFFER_SIZE ){
      if (pn_spo2) *pn_spo2 =  -999 ; // do not use SPO2 since valley loc is out of range
      if (pch_spo2_valid) *pch_spo2_valid  = 0; 
      return;
    }
  }
  // find max between two valley locations and compute ratio
  for (k=0; k< n_exact_ir_valley_locs_count-1; k++){
    ...
    if (an_ir_valley_locs[k+1]-an_ir_valley_locs[k] >3){
      ...
      n_y_ac=  an_y[n_y_dc_max_idx] - n_y_ac;    // subtract linear DC
      n_x_ac= (an_x[an_ir_valley_locs[k+1]] - an_x[an_ir_valley_locs[k] ] )*(n_x_dc_max_idx -an_ir_valley_locs[k]); // ir
      n_x_ac=  an_x[an_ir_valley_locs[k]] + n_x_ac/ (an_ir_valley_locs[k+1] - an_ir_valley_locs[k]); 
      n_x_ac=  an_x[n_y_dc_max_idx] - n_x_ac;      // subtract linear DC
  ...
  if( n_ratio_average>2 && n_ratio_average <184){
    ...
    if (pn_spo2) *pn_spo2 = n_spo2_calc ;
    if (pch_spo2_valid) *pch_spo2_valid  = 1;
  }
  else{
    if (pn_spo2) *pn_spo2 =  -999 ;
    if (pch_spo2_valid) *pch_spo2_valid  = 0; 
